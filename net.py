...
class CRNN(nn.Module):
    ...
    def forward(self, input):
        ...
        b, c, h, w = conv.size()
        conv = conv.squeeze(2)
        conv = conv.permute(0, 2, 1)    # torch.Size([256, 10, 64])
        output = self.rnn(conv) # torch.Size([256, 10, 64])
        soh = self.soh(output).squeeze() # torch.Size([256, 10])
        
        if not self.sigmoid:
            rul = self.rul(soh) # torch.Size([256, 1])
        else:
